...
SocketAddress SocketAddress::CreateTCPSocket(std::string host, UINT16 portNum)
{
	...
	sockAddr.sin_family = AF_INET;
	sockAddr.sin_port = htons(portNum);
	InetPton(AF_INET, (PCWSTR)host.c_str(), &sockAddr.sin_addr);
...
int TCPSocket::Bind(std::string host, UINT16 portNum)
{
	sockAddr = SocketAddress::CreateTCPSocket(host, portNum);
	if (::bind(sock, (SOCKADDR*)&sockAddr, sockAddr.addrLen) == SOCKET_ERROR)
	{
		SocketUtil::GetLastSocketError();
		return SOCKET_FAIL;
